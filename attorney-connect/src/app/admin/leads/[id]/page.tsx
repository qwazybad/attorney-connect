...
type Lead = {
  ...
  created_at: string;
  attorney: { id: string; name: string | null; firm: string | null; photo_url: string | null } | null;
};
...
function StatusBadge({ status }: { status: LeadStatus | null }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 text-sm font-semibold px-3 py-1.5 rounded-full border ${s.bg} ${s.border} ${s.color}`}>
      {s.label}
    </span>
  );
}
  ...
  if (!lead) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-3">
        ...
        <Link href="/admin/leads" className="text-blue-500 text-sm hover:underline">Back to pipeline</Link>
      </div>
    );
  }
      ...
      <header className="sticky top-0 z-40 bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-5xl mx-auto px-5 sm:px-8 h-16 flex items-center justify-between">
          ...
          <Link href="/admin/leads" className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-900 transition-colors">
            <ArrowLeft className="w-4 h-4" /> Leads Pipeline
          </Link>
        </div>
      </header>
      ...
      <div className="max-w-5xl mx-auto px-5 sm:px-8 py-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-0.5">{lead.legal_issue} · {lead.state} · submitted {fmtFull(lead.created_at)}</p>
                {lead.attorney && (
                  <p className="text-sm mt-1">
                    <span className="text-gray-400">Attorney: </span>
                    <Link href={`/admin/attorneys/${lead.attorney.id}`} className="text-blue-600 hover:underline font-semibold">
                      {lead.attorney.name ?? lead.attorney.firm ?? "Unknown"}
                    </Link>
                    {lead.attorney.firm && lead.attorney.name && <span className="text-gray-400"> · {lead.attorney.firm}</span>}
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Lead ID</p>
                  <p className="text-xs text-gray-400 font-mono break-all">{lead.id}</p>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-2 bg-white border border-gray-200 rounded-2xl shadow-sm p-6">
              <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">Funnel Answers</h2>
              {lead.message ? (
                <div className="divide-y divide-gray-100">
                  {parseMessage(lead.message)}
                </div>
              ) : (
                <p className="text-sm text-gray-400 italic">No funnel answers recorded.</p>
              )}
            </div>
          </div>
        )}
        ...
        {tab === "documents" && (
          <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6">
            ...
            <DocumentsPanel
              attorneyId={lead.attorney?.id ?? null}
              leadId={lead.id}
              canUpload
              canDelete
            />
